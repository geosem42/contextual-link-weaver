...
class Interpost_Suggestions {
	...
	public static function get_suggestions( $draft_content, $current_post_id ) {
		...
		$draft_embedding = Interpost_Embeddings::call_embedding_api( $plain_text, 'RETRIEVAL_QUERY' );
		if ( is_wp_error( $draft_embedding ) ) {
			return $draft_embedding;
		}

		// 2. Find similar posts.
		$similar_posts = Interpost_Embeddings::find_similar_posts( $draft_embedding, $current_post_id, 15 );

		if ( empty( $similar_posts ) ) {
			return new WP_Error( 'no_similar_posts', __( 'No indexed posts found. Please index your posts first from the settings page.', 'interpost' ) );
		}
		...
		if ( empty( $post_list ) ) {
			return new WP_Error( 'no_posts', __( 'Could not load post data for similar posts.', 'interpost' ) );
		}
		...
		if ( ! is_array( $raw_suggestions ) ) {
			return new WP_Error( 'invalid_response', __( 'API returned a non-array response.', 'interpost' ) );
		}
...
If you cannot find any good matches that follow all the rules, return an empty array [].";
	...
	private static function call_generative_api( $prompt ) {
		...
		$model_id = apply_filters( 'interpost_generative_model', 'gemini-3.7-flash' );
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error( 'generative_api_error', sprintf( __( 'Gemini API returned status %d', 'interpost' ), $code ), $body );
		}
		...
		if ( ! $generated_text ) {
			return new WP_Error( 'invalid_response', __( 'Could not find generated text in API response.', 'interpost' ) );
		}

		$parsed = json_decode( $generated_text, true );
		if ( json_last_error() !== JSON_ERROR_NONE ) {
			return new WP_Error( 'json_parse_error', __( 'Failed to parse JSON from Gemini response.', 'interpost' ) );
		}
